...
<?php foreach ($images as $index => $image): ?>
    <article class="gallery-card image-item" data-lightbox-index="<?= (int) $index ?>" data-display-src="/image/display/<?= (int) $image['id'] ?>" data-alt="<?= e((string) (($locale === 'en' ? $image['alt_en'] : $image['alt_es']) ?: 'Photo')) ?>">
        <canvas class="protected-canvas" width="400" height="267"></canvas>
        <img src="/image/thumb/<?= (int) $image['id'] ?>" alt="<?= e((string) (($locale === 'en' ? $image['alt_en'] : $image['alt_es']) ?: 'Photo')) ?>" draggable="false" class="source-image" loading="eager">
